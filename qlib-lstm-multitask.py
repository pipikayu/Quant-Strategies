...
def create_sequences(data, seq_length):
    ...
    return np.array(X), np.array(y_price), np.array(y_trend)
...
if __name__ == "__main__":  # 确保 main 函数在脚本直接运行时执行
    ...
    X, y_price, y_trend = create_sequences(scaled_data, SEQ_LENGTH)
    ...
    def train_model(model, X_train, y_train_price, y_train_trend, criterion_price, criterion_trend, optimizer, epochs, batch_size):
        ...
        for epoch in range(epochs):
            for i in range(0, len(X_train), batch_size):
                ...
                loss = loss_price + 0.05*loss_trend
    ...
    print(f"Trend Prediction Accuracy: {accuracy:.5f}")
    ...
    #plt.gcf().text(0.15, 0.75, metrics_text, fontsize=10, bbox=dict(facecolor='white', alpha=0.5))
