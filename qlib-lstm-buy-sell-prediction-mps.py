...
def train_tcts_model(model, X_train, y_train, criterion, optimizer, epochs, batch_size):
    ...
    for epoch in range(epochs):
        for i in range(0, len(X_train), batch_size):
            X_batch = X_train[i:i + batch_size].to(device)  # Move to MPS device
            y_batch = y_train[i:i + batch_size].to(device)  # Move to MPS device
...
if __name__ == "__main__":  # 确保 main 函数在脚本直接运行时执行
    ...
    X_train_torch = torch.tensor(X_train, dtype=torch.float32).to(device)  # 修改为 float32
    y_train_torch = torch.tensor(y_train, dtype=torch.long).to(device)  # 使用long进行分类
    X_test_torch = torch.tensor(X_test, dtype=torch.float32).to(device)  # 修改为 float32
    y_test_torch = torch.tensor(y_test, dtype=torch.long).to(device)  # 使用long进行分类
    ...
    with torch.no_grad():
        ...
        accuracy = accuracy_score(y_test, predicted.cpu().numpy())  # Move to CPU before converting to numpy
    ...
    # 创建集成模型
    ensemble_model = EnsembleModel(lstm_model, tcts_model, INPUT_SIZE, OUTPUT_SIZE).to(device)
    ...
    def train_ensemble_model(model, X_train, y_train, criterion, optimizer, epochs, batch_size):
        ...
        for epoch in range(epochs):
            for i in range(0, len(X_train), batch_size):
                X_batch = X_train[i:i + batch_size].to(device)  # 移动到 MPS 设备
                y_batch = y_train[i:i + batch_size].to(device)  # 移动到 MPS 设备
    ...
    ensemble_model.eval()
    with torch.no_grad():
        outputs_ensemble = ensemble_model(X_test_torch.to(device))  # 确保输入数据在 MPS 设备上
