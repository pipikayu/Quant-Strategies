...
import re  # 导入正则表达式模块
from sklearn.metrics import accuracy_score, classification_report  # Add this import statement
...
def prepare_data(data, ticker, output_dir):
    ...
    data.loc[:, 'future_min_close'] = data['Close'].rolling(window=5).min()  # Use .loc to avoid SettingWithCopyWarning
    # 反转数据以恢复原始顺序
    data = data.iloc[::-1]
...
def train_agent(data, features, initial_balance=100000.0, transaction_cost=0.001, train_split=0.8, model_save_path="xgboost_model.json"):
    ...
    logging.info(f"Training Accuracy: {train_accuracy:.2f}")  # Log training accuracy

    # Calculate and log classification report for each class
...
def backtest(model, test_data, selected_features, initial_balance=100000.0, transaction_cost=0.001, model_path="xgboost_model.json", output_dir="./", ticker=""):
    ...
    # 保存最近5天的信号到文件
    recent_signals = []  # 初始化一个空列表，用于存储最近5天的信号
...
if __name__ == "__main__":
    ...
    data, features = prepare_data(data, ticker, output_dir)
